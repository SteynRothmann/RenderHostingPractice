...
// Bump this whenever seed() changes in a way that should override data
// already saved in someone's browser (like clearing the example chats/
// groups/notifications below) - load() only re-seeds when this exact key
...
function seed(): AppState {
  return {
    ...
    // These three were seeded with example data for early testing/practice
    // only. Notifications are now real chat requests against real Spotify
    // accounts (see backend/lib/chatRequests.js + NotificationsPanel), so
    // there's nothing to seed here anymore. Chats/groups are still mock/
    // local-only (chat itself isn't built yet), so they start empty too.
    chats: {},
    groups: {},
    notifications: [],
    hasNotifDot: false,
